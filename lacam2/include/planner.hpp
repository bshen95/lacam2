...
struct MCTNode {
    ...
    double compute_uct_value(HNode* goal_node) {
    ...
    double print_uct_value(HNode* goal_node) {
      double exploitation = reward / (visits + 1e-6);
      // std::cout<< " "<< std::endl; 
      // std::cout<< std::to_string(parent == nullptr ? 0 : parent->visits) << std::endl;
      // std::cout<< visits << std::endl; 
...
struct Planner {
  ...
  double downward_rollout_policy(HNode* H, HNode*& H_goal, std::unordered_map<Config, HNode*, ConfigHasher>& EXPLORED, int num_sample, int max_depth);
  ...
  void MCT_backpropagate(MCTNode* mct_node, double reward);
  int get_rollout_increase();
  int get_makespan_lower_bound(const Config& C);
  void print_utc_value(std::vector<MCTNode*>& node_pool, HNode* goal_node);
